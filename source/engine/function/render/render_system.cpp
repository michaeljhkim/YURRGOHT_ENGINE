...
#include "engine/function/render/pass/directional_light_shadow_pass.h"
#include "engine/function/render/pass/point_light_shadow_pass.h"
#include "engine/function/render/pass/spot_light_shadow_pass.h"
...
namespace Yurrgoht {
	...
	void RenderSystem::init() {
		...
		g_engine.eventSystem()->addListener(EEventType::RenderCreateSwapchainObjects, 
			std::bind(&RenderSystem::onCreateSwapchainObjects, this, std::placeholders::_1));
		g_engine.eventSystem()->addListener(EEventType::RenderDestroySwapchainObjects,
			std::bind(&RenderSystem::onDestroySwapchainObjects, this, std::placeholders::_1));
		g_engine.eventSystem()->addListener(EEventType::RenderRecordFrame,
			std::bind(&RenderSystem::onRecordFrame, this, std::placeholders::_1));
		g_engine.eventSystem()->addListener(EEventType::PickEntity,
			std::bind(&RenderSystem::onPickEntity, this, std::placeholders::_1));
		g_engine.eventSystem()->addListener(EEventType::SelectEntity,
	...
	void RenderSystem::collectRenderDatas() {
		...
		// infinite xz grid 
		std::shared_ptr<InfiniteGridRenderData> infinite_grid_data = std::make_shared<InfiniteGridRenderData>();
		infinite_grid_data->view_uniforms = { 
			camera_component->getViewMatrix(),
			camera_component->getProjectionMatrix(),
			glm::vec3(0, 0, 0)
		};
		...
		m_main_pass->setInfiniteGridRenderDatas(infinite_grid_data);
