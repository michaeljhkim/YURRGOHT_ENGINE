...
namespace Yurrgoht {
	...
	void LogUI::init() {
        std::cout << "LogUI::init (no LOG_INFO)" << std::endl;
		m_title = "Log";
	}
	...
	void LogUI::showLogText(const std::string& log) {
		...
		for (int i = 0; i <= (int)(ELogLevel::Fatal); ++i) {
			...
			if (std::string(log.begin() + 1, log.begin() + log_str_len + 1) == k_log_strs[log_level]) {
				...
				ImGui::Text("%s", (std::string("[") + k_log_strs[log_level] + "]").c_str());
				ImGui::PopStyleColor();

				ImGui::SameLine();

				//size_t second_right_bracket_pos = find_nth(log, "]", 2);
				//ImGui::Text("%s", std::string(log.begin() + second_right_bracket_pos + 2, log.end()).c_str());
				size_t first_right_bracket_pos = find_nth(log, "]", 1);
				ImGui::Text("%s", std::string(log.begin() + first_right_bracket_pos + 2, log.end()).c_str());
			}
		}
	}
